...
import { parseGeoPackage, validateGeoPackageFile, calculateLayerStats } from '../../services/geopackageService';
import { saveGeoPackage } from '../../services/adminService';
import { useAuth } from '../../contexts/AuthContext';

export default function GeoPackageUploader({ onUploadComplete }) {
  ...
  const handleFileSelect = async (event) => {
    ...
    // Validate file
    const validation = validateGeoPackageFile(file);
    if (!validation.valid) {
    ...
    if (validation.warnings.length > 0) {
      console.warn('GeoPackage warnings:', validation.warnings);
    }
    ...
    try {
      ...
      const data = await parseGeoPackage(file);
      setProgress(60);
      ...
      setError(err.message || 'Failed to parse GeoPackage file');
    } finally {
      setUploading(false);
    }
  };
    ...
    <Card>
      <CardContent>
        <Box display="flex" alignItems="center" gap={1} mb={2}>
          ...
          <Typography variant="h6">Upload GeoPackage File</Typography>
        </Box>

        <Typography variant="body2" color="text.secondary" paragraph>
          Upload a .gpkg file to add spatial data layers to your map. Supports points, lines, and polygons.
        </Typography>
        ...
        <Box mb={2}>
          <input
            accept=".gpkg"
            style={{ display: 'none' }}
            id="geopackage-file-input"
            type="file"
            onChange={handleFileSelect}
            disabled={uploading}
          />
          <label htmlFor="geopackage-file-input">
            <Button
              variant="contained"
              component="span"
        ...
        {success && (
          <Alert severity="success" icon={<CheckCircleIcon />} sx={{ mb: 2 }}>
            GeoPackage uploaded successfully!
          </Alert>
        )}
